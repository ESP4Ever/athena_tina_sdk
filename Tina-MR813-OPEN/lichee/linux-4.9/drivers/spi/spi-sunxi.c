...
#define SPI_NO_THREAD_SPINLOCK      /* try turnon this if kenerl panic */
...
#ifdef SPI_NO_THREAD_SPINLOCK
	raw_spinlock_t slock; /* syn */
#endif
	spinlock_t lock;
...
static void spi_set_dbi_clk_mode(void __iomem *base_addr)
{
	u32 reg_val = readl(base_addr + SPI_DBI_CR_REG1);
...
static void spi_config_dbi(struct spi_device *spi, void __iomem *base_addr)
{
	...
	u32 reg_val = readl(base_addr + SPI_DBI_CR_REG);
	u32 reg_tmp = 0;
	...
	reg_val = readl(base_addr + SPI_DBI_CR_REG2);
	...
	reg_val = readl(base_addr + SPI_DBI_CR_REG1);
...
static irqreturn_t sunxi_spi_handler(int irq, void *dev_id)
{
...
#ifdef SPI_NO_THREAD_SPINLOCK
	raw_spin_lock_irqsave(&sspi->slock, flags);
#else
	spin_lock_irqsave(&sspi->lock, flags);
#endif
...
#ifdef SPI_NO_THREAD_SPINLOCK
			raw_spin_unlock_irqrestore(&sspi->slock, flags);
#else
			spin_unlock_irqrestore(&sspi->lock, flags);
#endif
			return IRQ_HANDLED;
	...
	if (sspi->dbi_enabled) {
		...
		dprintk(DEBUG_INFO, "[dbi%d] irq status = %x\n",
				sspi->master->bus_num, status);
		if ((status & DBI_INT_STA_FRAME) || (status & DBI_INT_FIFO_EMPTY)) {
			dprintk(DEBUG_INFO, "[spi%d] DBI Fram TC comes\n",
					sspi->master->bus_num);
			dbi_disable_irq(DBI_FRAM_DONE_INT_EN |
					    DBI_FIFO_EMPTY_INT_EN,
					base_addr);
			/*wakup uplayer, by the sem */
			complete(&sspi->done);
#ifdef SPI_NO_THREAD_SPINLOCK
		...
		struct sunxi_spi_platform_data *pdata, struct device *dev)
{
	...
	if (sspi->dbi_enabled) {
		...
		spi_set_dbi_clk_mode(base_addr);
		spi_enable_tp(base_addr);
	} else if (!sspi->mode) {
...
static int sunxi_spi_probe(struct platform_device *pdev)
{
	...
	err = request_irq(sspi->irq, sunxi_spi_handler, IRQF_NO_THREAD, sspi->dev_name, sspi);
	if (err) {
	...
	if (sspi->mode) {
		...
		sspi->workqueue = alloc_workqueue(dev_name(master->dev.parent), WQ_MEM_RECLAIM | WQ_HIGHPRI | WQ_CPU_INTENSIVE, 0);
