...
static ssize_t giga_test_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct net_device *ndev = dev_get_drvdata(dev);
...
static ssize_t giga_test_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct net_device *ndev = dev_get_drvdata(dev);
	struct geth_priv *priv = netdev_priv(ndev);
	...
	data2 = sunxi_mdio_read(priv->base, priv->phy_addr, MII_CTRL1000);
	pr_info("read MII_CTRL1000(0x09) Reg: 0x%x\n", data2);
	data1 = sunxi_mdio_read(priv->base, priv->phy_addr, 0x1E);
	pr_info("read page select register(0x1E) Reg: 0x%x\n", data1);

	ret = kstrtou16(buf, 0, &value);
	if (ret)
		return ret;
	//pr_info("buf: 0x%x, value: 0x%x\n", buf, value);

	if (value == 0) {
		data1 &= 0x0000;
		data2 &= 0x0000;
		sunxi_mdio_write(priv->base, priv->phy_addr, MII_CTRL1000, data2);
		pr_info("Set MII_CTRL1000(0x09) Reg: 0x%x\n", data2);
		sunxi_mdio_write(priv->base, priv->phy_addr, 0x1E, data1);
		pr_info("page select register(0x1E) Reg: 0x%x\n", data1);
	} else if (value == 1) {
		data1 &= 0x0000;
		data2 &= ~(0x7 << 13);
		data2 |= value << 13;
...
static DEVICE_ATTR(giga_test_mode, 0664, giga_test_mode_show, giga_test_mode_store);
...
static int geth_probe(struct platform_device *pdev)
{
	...
	device_create_file(&pdev->dev, &dev_attr_giga_test_mode);
...
static int geth_remove(struct platform_device *pdev)
{
	...
	device_remove_file(&pdev->dev, &dev_attr_giga_test_mode);
